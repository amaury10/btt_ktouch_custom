...
{
    /* Sous 1 %, le rapport explose et produirait une valeur absurde --
     * mieux vaut ne rien afficher. À 100 % ou au-delà, plus de temps
     * restant a estimer. */
    if (!(progression > 0.01f && progression < 1.0f && ecoule_s > 0.0f)) {
        return 0;
    }

    float restant = ecoule_s * (1.0f - progression) / progression;
     ...
     *   On NE plafonne PAS : le résultat reste 0, qui est justement le sens
     *   documenté de ce champ (« 0 si inconnu », voir etat_klipper.h).
     *   Plafonner ici transformerait une entrée invalide en une estimation
     *   d'allure plausible.
     *
...
bool moonraker_parse_status(const char *json, size_t longueur, etat_klipper_t *sortie)
{
    ...
    /* Moonraker ne fournit pas de temps restant : on l'estime à partir du
     * temps écoulé rapporté à la progression. Formule extraite dans
     * moonraker_estimer_temps_restant_s() (voir moonraker_parse.h) --
     * partagée avec rpc_fusionner_status()/rpc_fusionner_instantane()
     * (moonraker_rpc.c), qui l'appellent avec les mêmes deux grandeurs
     * reçues en message séparés côté WebSocket. */
    e.temps_restant_s = moonraker_estimer_temps_restant_s(e.progression, ecoule);
