 ...
 * pile mbedtls) plus l'analyse cJSON du corps de réponse et le `etat_klipper_t
 * nouveau` que le backend pose sur cette pile (sizeof ~1808 octets), avec de
 * la marge ; tskIDLE_PRIORITY + 5 place cette tâche nettement au-dessus de
 * l'IDLE mais sans rivaliser avec les tâches internes du pilote WiFi ni la
 * tâche LVGL. */
